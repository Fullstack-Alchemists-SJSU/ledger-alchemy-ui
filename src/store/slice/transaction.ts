...
import { getTransactionsByUserIdService, readTransactionsByUserIdService, syncTransactionsByUserIdService } from '../../services/transaction';
...
export const getTransactionsByUserId = createAsyncThunk(
    ...
    async (userSub: string) => {
        try {
            const response = await getTransactionsByUserIdService(userSub);
...
export const syncTransactionsByUserId = createAsyncThunk(
    ...
    async (userSub: string) => {
        try {
            const response = await syncTransactionsByUserIdService(userSub);
...
export const readTransactionsByUserId = createAsyncThunk(
    ...
    async (userSub: string) => {
        try {
            const response = await readTransactionsByUserIdService(userSub);
            const transactions = response.data.map((transaction: Transaction) => ({
                account_id: transaction.account_id,
                account_owner: transaction.account_owner,
                amount: transaction.amount,
