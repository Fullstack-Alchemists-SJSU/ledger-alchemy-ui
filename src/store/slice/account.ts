...
export const createLinkToken = createAsyncThunk(
	...
	async (data: { userSub: string; token: string }) => {
		try {
			const response = await createLinkTokenService(data.userSub, data.token);
...
export const getAccessToken = createAsyncThunk(
	...
	async (data: { userSub: string; public_token: string; token: string }) => {
		try {
			const response = await getAccessTokenService(data.userSub, data.public_token, data.token);
...
export const getAccountsByUserId = createAsyncThunk(
	...
	async (data: { userSub: string; token: string }) => {
		try {
			const response = await getAccountsByUserIdService(data.userSub, data.token);
			const accounts = response.data.map((account: Account) => ({
				account_id: account.account_id,
				balances: account.balances,
				mask: account.mask,
